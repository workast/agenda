...
module.exports = function() {
  ...
  function computeFromRrule() {
    ...
    if (!rrule.dtstart && rrule.tzid && moment.tz(rrule.tzid).utcOffset() < 0) {
      rrule.dtstart = moment(moment.tz(rrule.tzid).format('YYYY-MM-DDTHH:mm:ss')).toDate();
    }
    const rruleObject = new RRule(rrule);
    let lastRunAt = this.attrs.lastRunAt || new Date();
    if (rrule.tzid && moment.tz(rrule.tzid).utcOffset() < 0) {
      // If there's a timezone set that is behind UTC, we have to hard-convert
      // lastRunAt to the specific timezone, and pass it to RRule as if it was UTC.
      // This is due to the way RRule processes dates with timezones, probably a bug.
      lastRunAt = moment(moment.tz(lastRunAt, rrule.tzid).format('YYYY-MM-DDTHH:mm:ss')).toDate();
      debug('[%s:%s] set lastRunAt to [%s]', this.attrs.name, this.attrs._id, lastRunAt.toISOString());
    }
